...
class Block(nn.Module):
    def __init__(self, channels):
        ...
        # with mnn.utils.make_partials(StdConv, MBConv, DSConv):
        self.op = spos.SelectOp(
            ...
            p(DSConv, kernel=1, expansion=2, channels=channels),
            ...
            p(StdConv, kernel=3, channels=channels),
...
class Reduction(nn.Module):
    def __init__(self, in_channels, out_channels, proj_3x3: bool = False):
        super().__init__()

        self.conv1 = GenericConv(in_channels, out_channels, 1, 1)
...
class SpeechCommandsModel(nn.Module):
    def __init__(self, stem=64, num_classes=10, blocks=[4,4,4,4], channels_scale=1.5, proj3x3:bool=False, **kwargs):
        super().__init__()

        in_ch = kwargs['input_size'][1]

        self.stem = GenericConv(in_ch, stem, 3, padding=1, stride=2)
        ...
        for num_blocks in blocks:
            ...
            layers.append(Reduction(channels, new_channels, proj3x3))
    ...
    def forward(self, x):
        if self.multi_task and self._task_id is None:
            raise RuntimeError('Forgot to set a task id?')
